...
char temp_n[]="";
    ...
    if(command.compare("\"getsuc\"")==0){
      ...
      // }
      // else{
     // node_info succ_node = node.findSuccessor(help.find_consitant_hash_id(value));
       if(help.check_string_for_number(key)==false){
           char resp[]="-1";
            write_response(fd,resp);
      ...
      node_info succ_node = node.findSuccessor(stoi(key));
      ...
      close(fd);
		...
		if(node_succ.id != node.current.id){
			string resp = node.establish_connection_and_forward_msg(node_succ.ip,node_succ.port,input.second);
