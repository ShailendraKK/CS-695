...
int save_to_file(char * file_name,char *data,int create_new){
    ...
    int temp;
    if (create_new){
        fd = open(file_name,O_CREAT| O_RDWR | O_TRUNC,S_IRWXU);
    }
    else{
...
char** get_server_ip_address(char *file_path){
    char *ip_addr[5];//= (char **)calloc(sizeof(char)*150,sizeof(char));
    int i=0;
    char *tmp;
...
int check_ip_removed(char ** ip1,char** ip2){
    if(ip2[0]==NULL){
        return -1;
    }
    for(int i=0;ip2[i]!=NULL;i++){
          if(ip1[i]==NULL){
            return i;
        }
        if(strcmp(ip1[i],ip2[i])!=0){
