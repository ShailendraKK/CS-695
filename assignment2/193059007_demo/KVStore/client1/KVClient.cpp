...
int handle(){
		...
		for(int i=0;i<strlen(command); i++)
       {
		...
		strcpy(resp,"");
		FD_ZERO(&set);
        FD_SET(sock_fd[temp_index],&set);

        timeout.tv_sec = 0;
        timeout.tv_usec = 500000;
		int rv = select(sock_fd[temp_index]+1,&set,NULL,NULL,&timeout);
		if(rv == -1)
  			  return 0; 
  		else if(rv == 0){
...
void input_mode(){
	...
	cin >> mode;
	fflush(stdout);
	(mode==0)?cout<<"Enter low load mode":cout<<"Enterting high load mode";
	}
}
...
int main(int argc, char *argv[])
{
	...
	char ip[]="localhost";
   ip_fd =connect_to_socket(ip,8090);

int ip_rem_index=0;
thread input_thread(input_mode);
input_thread.detach();
	while(1){
	fseek(fin, 0, SEEK_SET);
	while(fgets(input,262404,fin)!=NULL){
		//printf("Getting server ips from DNS\n");
	...
	int check=send(ip_fd,send_char_data,strlen(send_char_data), MSG_NOSIGNAL);
		...
		save_to_file(file_name,resp,1);

		//printf("Got server ips from DNS\n");
		
        int ip_addr_index=0;
		char *tmp;
		char *filecontent = read_from_file(file_name,161);
		while((tmp =__strtok_r(filecontent,"\n",&filecontent))){
       
        ip_addr[ip_addr_index]=tmp;
        ip_addr_index++;
        
        }
		for(int temp=ip_addr_index;temp<5;temp++){
			ip_addr[temp]=NULL;
		}
		//ip_addr= get_server_ip_address("server_ip");
		if((ip_rem_index=check_ip_removed(ip_addr,server_ip))!=-1){
			remove_ip(ip_rem_index);
			//printf("checked for removal of ips\n");
		}
		else if(check_new_ip_added(ip_addr,server_ip)==1 && ip_addr[index_ip]!=NULL){
			//ip_addr2[index_ip]=get_new_server_ip(ip_addr,ip_addr2);
			
			server_ip[index_ip]=ip_addr[index_ip];//get_new_server_ip(ip_addr,server_ip);
			
			//printf("Connecting to %s\n",server_ip[index_ip]);
			sock_fd[index_ip] =connect_to_socket(server_ip[index_ip],8080);
			index_ip++;
			//printf("checked for addition of ips\n");
		}
